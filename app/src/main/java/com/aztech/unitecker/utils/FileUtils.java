...
public class FileUtils {
    ...
    public static String generateRandomIdentifier() {
        ...
        for (int i = 0; i < ConstantsData.STICKER_PACK_IDENTIFIER_LENGHT; i++) {
    ...
    public static void initializeDirectories(Context context) {
        File directory = new File(ConstantsData.STICKERS_DIRECTORY_PATH);
        if (!directory.exists()) {
            directory.mkdir();
            String value = "{\"androidPlayStoreLink\": \"\",\"iosAppStoreLink\": \"\",\"stickerPacks\": [ ]}";
            try {
                PrintWriter out = new PrintWriter(ConstantsData.STICKERS_DIRECTORY_PATH + "/contents.json");
                out.write(value);
                out.close();
